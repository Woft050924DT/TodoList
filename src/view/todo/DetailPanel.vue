<template>
  <aside class="w-72 shrink-0 bg-white border-l border-gray-100 flex flex-col">
    <div
      class="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-100"
    >
      <span class="text-sm font-semibold text-gray-700">Chi tiết</span>
      <button
    ...
    <div class="flex-1 overflow-y-auto px-4 py-3 space-y-4">
      ...
      <div>
        <div
          class="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-1"
        >
          Tiêu đề
        </div>
        <p
      ...
      <div class="flex gap-3">
        <div class="flex-1">
          ...
          <select
            ...
            class="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-violet-300 transition"
        ...
        <div class="flex-1">
          ...
          <select
            ...
            class="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-xs text-gray-600 focus:outline-none focus:ring-2 focus:ring-violet-300 transition"
          >
      ...
      <div>
        ...
        <div
          class="flex items-center gap-2 border border-gray-200 rounded-lg px-2.5 py-1.5"
        >
          <svg
            width="13"
            height="13"
          ...
          <input
            ...
            class="text-xs text-gray-600 focus:outline-none flex-1 bg-transparent"
          />
        </div>
        <p
          v-if="deadlineLabel && !todo.completed"
          class="text-[10px] mt-1 font-semibold flex items-center gap-1"
      ...
      <div>
        ...
        <div class="space-y-1.5">
          ...
          >
            ...
            <span
              ...
              :class="sub.done ? 'line-through text-gray-400' : 'text-gray-600'"
              >{{ sub.title }}</span
            >
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 mt-2">
          <input
            ...
            class="flex-1 text-xs border-b border-gray-200 py-1 focus:outline-none focus:border-violet-400 text-gray-600 placeholder-gray-300 transition bg-transparent"
          />
          <button
            @click="submitSubtask"
      ...
      <div>
        <div
          class="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-2"
        >
          Thẻ nhãn
        </div>
        <div class="flex flex-wrap gap-1.5 mb-2">
          <span
            ...
            :key="tag"
            class="flex items-center gap-1 px-2 py-0.5 bg-violet-50 text-violet-600 rounded-full text-[10px] font-medium"
          >
            {{ tag }}
            <button
    ...
    <div class="border-t border-gray-100 p-3">
      <button
        ...
        class="w-full flex items-center justify-center gap-2 py-2 text-xs font-semibold text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"
      >
        <svg
          width="13"
          height="13"
...
function updateCategory(e: Event) {
  const t = store.todos.find((t) => t.id === props.todo.id);
  if (t) t.category = (e.target as HTMLSelectElement).value as any;
}
